...
The repo provides the `json-schema-rs` library and the `jsonschemars` CLI. We target **JSON Schema Draft 2020-12**; default settings (e.g. `JsonSchemaSettings::builder().build()`) are tuned for that spec. A script downloads specs for every published draft (draft-00 through 2020-12); run `make vendor_specs` to fetch them locally—specs are not stored in the repo. Supported keywords include **type**, **properties**, **required**, and **enum** (string-only; codegen emits Rust enums). For implementation details and design philosophy, see [design.md](design.md). Generated
