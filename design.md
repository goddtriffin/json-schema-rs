...
Implemented keywords: type (object, string, integer, number, boolean, array), properties, required, title, enum, items, additionalProperties. Other keywords are documented in the sections below. Unknown schema keywords and property types that do not map to generated code are ignored and do not cause an error.
...
Performance is measured by the **`json_schema_rs_benchmark`** workspace crate (top-level, next to `json_schema_rs`; `publish = false`), driven by a single `make benchmark` target (`research_benchmark` is a backwards-compat alias). It compares our library against competitor tools on shared fixtures and marks a winner per metric category. Design:

- **Two measurement systems.** Our own in-process hot paths (parse→compile, per-instance validate on valid and invalid instances, codegen) are measured with **criterion** for low-variance, startup-free numbers used to gate our own regressions. Competitors run as **external CLIs** timed end-to-end; only **boon** is linked (via a small `boon_validate` wrapper bin) because it has no CLI. typify (`cargo typify`) and the Stranger6667 `jsonschema` CLI are the competitor CLIs; **schemafy has no CLI** (proc-macro only) and so is not a codegen competitor.
- **Required competitor tools (hard fail).** The driver aborts up front (with install hints) if any required competitor CLI — `cargo-typify` or `jsonschema-cli` — is missing, rather than silently omitting a competitor and misrepresenting the comparison. `scripts/tools.sh` owns the required set (`required_external_tools`) and the check (`check_required_tools`); our own bins (`jsonschemars`, `boon_validate`) are built by the driver, so they and `boon` are never in that set. This is distinct from **fixture-level** incompatibility, which is recorded as a compat failure rather than aborting.
- **Universal measurement.** Every CLI run is wrapped in `/usr/bin/time` (macOS `-l` / Linux `-v`), which yields both wall-time (`real`) and peak resident set size in one shot for every tool uniformly (no separate timing tool such as Hyperfine is used or required). Because our validate/codegen CLI re-parses per invocation, the schema-compile-vs-validate **split** is only measured in-process (criterion); cross-tool CLI numbers are end-to-end and startup-dominated for fast operations. A validation timing is recorded **only when the tool returns the correct verdict** — a valid payload accepted (exit 0), an invalid payload rejected (non-zero exit) — so a tool that errors or mis-validates (e.g. cannot handle the schema) is not timed as a misleadingly-fast validation.
- **Metrics (lower is better).** codegen wall-time, schema-compile time, per-instance validate (valid), per-instance validate (invalid), and peak memory. Duplicate `(tool, fixture, category)` samples are collapsed — min for times (best), max for peak memory (true footprint). Winner per (category, fixture) cell uses a 3% tie threshold; the per-category champion is the tool with the most outright cell wins.
- **Fixtures** live in `json_schema_rs_benchmark/fixtures/<size>/<name>/` (`schema.json` + `instances/valid/*` + `instances/invalid/*`) at small/medium/large/massive tiers, with a `manifest.json` recording provenance/license and which fixtures are **apples-to-apples** (handled cleanly by every tool). Real-world schemas come from SchemaStore, CircleCI, and kubernetes-json-schema; the massive tier includes a large generated schema. Any **valid** schema our library cannot handle is filed as an issue (e.g. #18, `allOf`-of-`$ref` codegen) and kept out of the apples-to-apples set.
- **Results are committed.** `make benchmark` writes `results.json` (machine-readable) and `results.md` (human-readable, winner per category + detailed fixture×tool tables + codegen compat matrix) into the crate, so a PR reviewer sees the performance impact in-diff. Per `CLAUDE.md`, **a PR is not complete until the harness has been run and these files are up to date.**
...
**Container/field attributes:** Container: **`#[json_schema(title = "...")]`** (when the schema had a title). Field-level **`#[json_schema(minimum = N, maximum = N)]`** is supported for emitting JSON Schema bounds on a property; N can be integer or float literals (stored as f64). When present, the attribute value overrides the type-derived minimum/maximum (e.g. an `i64` field with `#[json_schema(minimum = 0, maximum = 255)]` emits a schema with those bounds). Field-level **`#[json_schema(min_items = N, max_items = M)]`** is supported for array/set properties (Vec, HashSet, Option<Vec>, Option<HashSet>); when present, the attribute overlays the type-derived schema so the emitted JSON Schema includes minItems/maxItems. Field-level **`#[json_schema(min_length = N, max_length = M)]`** is supported for string properties (String, Option<String>); when present, the attribute overlays the type-derived schema so the emitted JSON Schema includes minLength/maxLength. Field-level **`#[json_schema(pattern = "...")]`** is supported for string properties; when present, the emitted JSON Schema includes the pattern (ECMA 262 regex). Field-level **`#[json_schema(deprecated = true)]`** is supported; when present, the emitted JSON Schema includes `"deprecated": true` for that property. Other field attributes (e.g. `#[json_schema(...)]`) are parsed for future use; only supported schema keywords (type, properties, required, title, minimum, maximum, min_items, max_items, min_length, max_length, pattern, deprecated) are emitted today. Attribute names follow a Serde-style pattern (container vs field). **No literal recursion** in `reverse_code_gen`: schema construction and serialization use iteration + stack where depth can be large (see design principle above).
...
| Single required boolean | Y | Y | Y | Y | Y |
| Single optional boolean | Y | Y | Y | Y | Y |
| Dedupe (two identical schemas) | Y | — | Y | Y | — |
| Dedupe nested output (output in submodule) | — | — | Y | Y | — |
...
| deprecated (property, struct; meta-data only; codegen emits #[deprecated]; reverse via #[json_schema(deprecated = true)]) | Y | Y | Y | Y | Y |
...
1. **Explicit types:** A field attribute such as `#[json_schema(any_of = (TypeA, TypeB))]` so derive builds a property schema with `any_of: Some(vec![TypeA::json_schema(), TypeB::json_schema(), ...])`.
2. **Enum inference:** Extend the enum derive to support enums with data and emit `anyOf` (one subschema per variant type); optionally allow a struct-field flag like `#[json_schema(any_of)]` to document enum-typed fields as `anyOf`.
...
**Our implementation:** We accept both a single type string and an array of type strings at parse time; we store and use only the **first** type. `object`, `string`, `integer`, `number`, and `boolean` drive codegen (integer emits `i64` / `Option<i64>`; number emits `f64` / `Option<f64>`; boolean emits `bool` / `Option<bool>`; no numeric constraints yet); other types are ignored for codegen but can be used for validation. We do not support draft-03 style array elements that are schema objects (we only interpret type-name strings). See schema model and parsing/constructors in `json_schema_rs/src/json_schema/json_schema.rs`, Rust codegen in `json_schema_rs/src/code_gen/rust_backend.rs`, and validator in `json_schema_rs/src/validator/mod.rs`. For type `"string"` specifically, see **7. Strings** below. For type `"boolean"`, see **type: "boolean"** in **8. Numbers** below.
...
**Spec audit (local specs under `specs/`):** In draft 2019-09 and 2020-12, `additionalProperties` is in the applicator vocabulary and accepts a schema (including boolean schemas: `true`/`false`). In draft-04 through draft-07 it appears in the validation/applicator meta-schema with the same meaning: value may be **absent**, **boolean**, or a **schema object**. When **absent**, draft-04 and later treat it as **allow** (no constraint on properties outside `properties`). When **false**, no property name outside `properties` (and outside `patternProperties` when present) is allowed. When **true**, same as absent. When a **schema**, each additional property's **value** must validate against that schema. We do not implement `patternProperties` yet, so "additional" means any key not in `schema.properties`.

**Our implementation:**

- **Schema model:** `additional_properties: Option<AdditionalProperties>` on `JsonSchema` with `AdditionalProperties::Allow` | `Forbid` | `Schema(Box<JsonSchema>)`; custom serde (missing → None, bool → Allow/Forbid, object → Schema). Serialization omits when Allow (default); emits `false` when Forbid and the schema when Schema.
- **Validator:** For object instances, compute keys not in `schema.properties` (no patternProperties yet). None/Allow: no action. Forbid: push `ValidationError::DisallowedAdditionalProperty { instance_path, property }` per additional key. Schema(s): push (sub_schema, value, path) for each additional key for recursive validation.
- **Codegen:** Forbid → `#[serde(deny_unknown_fields)]` on the struct. Schema(sub) → extra field `#[serde(default)] pub additional: BTreeMap<String, T>` where T is the Rust type for sub (or `serde_json::Value` fallback for complex/unsupported sub). Allow/absent: no attribute, no extra field. **DedupeKey** includes `additional_properties` so structs differing only by this keyword are not deduped.
- **Reverse codegen:** Closed structs (no catch-all map field) emit `additionalProperties: false` in the derive. `BTreeMap<String, V>` implements `ToJsonSchema` and emits `type: "object"` with `additionalProperties: V::json_schema()`.
- **allOf merge:** When allOf merges object schemas, additionalProperties is not yet merged in a special way; the merged object schema takes the first/merged properties and required; additionalProperties behavior for merged allOf may be extended later.

**Spec version quirks:** Draft-03 and earlier may define or default `additionalProperties` differently; when supporting those drafts, confirm against local specs and add version-specific behavior if needed. For draft-04 through 2020-12 we implement: absent/true = allow, false = forbid, schema = validate each additional property value.
...
**Our implementation:** We parse and store `min_items` and `max_items` as `Option<u64>`. **Validator:** When `type_ == "array"` and the instance is an array, we check length against `min_items` (if present) and `max_items` (if present); we push `ValidationError::TooFewItems` or `TooManyItems` and continue (no fail-fast). Length bounds apply to all arrays regardless of `uniqueItems` (i.e. both Vec and HashSet in codegen). **Codegen:** Emitted Rust type remains `Vec<T>` or `HashSet<T>` (and optional variants) per existing rules; when the schema has `min_items` or `max_items`, we emit `#[json_schema(min_items = N, max_items = M)]` on the generated field (only the keys that are present) so generated models round-trip and reverse codegen reproduces the schema. **DedupeKey** includes `min_items` and `max_items` so array/set schemas with different bounds are not deduplicated. **Reverse codegen:** `Vec<T>::json_schema()` and `HashSet<T>::json_schema()` set `min_items: None`, `max_items: None`; the derive supports `#[json_schema(min_items = N, max_items = M)]` on array and set fields (Vec, HashSet, Option<Vec>, Option<HashSet>) and overlays them on the property schema.
...
**Our implementation:** We parse both a single type string `"string"` and an array whose first element is `"string"` (we store the first type only; see **4. type**). Validation: when `type_ == Some("string")`, we require the instance to be a JSON string (`instance.is_string()`); non-strings produce `ValidationError::ExpectedString`. String length constraints (`minLength`, `maxLength`) and **pattern** are applied when the instance is a string (see **minLength / maxLength** and **pattern** below). Codegen: properties with `type: "string"` emit Rust `String` or `Option<String>` (with `#[serde(rename = "...")]` when the field name differs from the JSON key; `#[json_schema(min_length = N, max_length = N)]` and/or `#[json_schema(pattern = "...")]` when constraints are present). See schema model `is_string()` and parsing in `json_schema_rs/src/json_schema/json_schema.rs`, validator in `json_schema_rs/src/validator/mod.rs`, and Rust backend in `json_schema_rs/src/code_gen/rust_backend.rs`. Reverse codegen: Rust `String` emits `"type": "string"` with no length or pattern bounds. Field-level `#[json_schema(min_length = N, max_length = N)]` and `#[json_schema(pattern = "...")]` attributes on String fields wire constraints through the derive macro.
...
- **Codegen (JSON Schema → Rust):** When a string property schema has `min_length` and/or `max_length`, the backend emits `#[json_schema(min_length = N)]`, `#[json_schema(max_length = N)]`, or `#[json_schema(min_length = N, max_length = N)]` before the field declaration. This is consistent with the `min_items`/`max_items` pattern for array fields.
- **Reverse codegen (Rust → JSON Schema):** `String::json_schema()` returns `"type": "string"` with no length constraints. Field-level `#[json_schema(min_length = N, max_length = N)]` on a `String` (or `Option<String>`) field feeds into `property_inserts` in the derive macro, producing a schema with the specified `minLength`/`maxLength`. The `.or(base.min_length)` / `.or(base.max_length)` merge means field attributes take precedence over base type defaults (which are `None`).
- **Macro (`json_schema_to_rust!`):** The macro generates Rust source with the `#[json_schema(...)]` attributes on string fields; when the generated code is compiled, the `ToJsonSchema` derive re-derives the schema (round-trip verified in the test suite).
...
The `pattern` keyword is a string value interpreted as an **ECMA 262** (JavaScript) regular expression. The instance string is valid if the pattern matches **anywhere** in the string (partial match); authors often use `^...$` for full-string match. Pattern is string-only; it is ignored when the instance is not a string.

**Our implementation:**
- **Schema model:** `JsonSchema.pattern: Option<String>` (serialized as `"pattern"`). `None` by default; omitted from serialized JSON when absent.
- **Validator:** In the `Some("string")` arm, after `min_length`/`max_length` checks, if `schema.pattern.is_some()`, we compile the pattern with the **regress** crate (ECMA 262–compliant). On compile failure we push `ValidationError::InvalidPatternInSchema`. On success we run the regex against the instance string; if it does not match (partial match is success), we push `ValidationError::PatternMismatch`. Pattern is only checked when the instance is a string; non-string instances get only `ExpectedString`.
- **Codegen (JSON Schema → Rust):** When a string property schema has `pattern`, the backend emits `#[json_schema(pattern = "...")]` (value escaped for Rust string literals). Same pattern as `min_length`/`max_length`.
- **Reverse codegen (Rust → JSON Schema):** `String::json_schema()` returns `"type": "string"` with no pattern. Field-level `#[json_schema(pattern = "...")]` on a `String` (or `Option<String>`) field feeds into `property_inserts`, producing a schema with the specified `pattern`.
- **allOf merge:** When merging two string subschemas, if both have `Some(pattern)` and the values differ, we return `CodeGenError::AllOfMergeConflictingPattern`; otherwise we take the one that is `Some` when the other is `None`.

**Spec version quirks:** Pattern is defined from draft-03 onward; semantics (ECMA 262, partial match) are consistent across drafts we support. No draft-specific settings required.
...
When a schema has `"type": "boolean"` (or `"type": ["boolean", ...]` with boolean first), the instance must be a JSON boolean (`true` or `false`). This meaning is **identical across all JSON Schema drafts** (draft-00 through 2020-12).

**Our implementation:** We parse both a single type string `"boolean"` and an array whose first element is `"boolean"` (we store the first type only; see **4. type**). Validation: when `type_ == Some("boolean")`, we require the instance to be a JSON boolean (`instance.is_boolean()`); non-booleans produce `ValidationError::ExpectedBoolean`. Codegen: properties with `type: "boolean"` emit Rust `bool` or `Option<bool>` (required vs optional). Array items with `type: "boolean"` emit `Vec<bool>` or `HashSet<bool>` when `uniqueItems: true`. Reverse codegen: `ToJsonSchema for bool` already emits `"type": "boolean"`. See schema model `is_boolean()` in `json_schema_rs/src/json_schema/json_schema.rs`, validator in `json_schema_rs/src/validator/mod.rs`, and Rust backend in `json_schema_rs/src/code_gen/rust_backend.rs`.

**Spec version quirks:** None for the **meaning** of type `"boolean"` across drafts. The only draft differences are in the **form** of the `type` keyword (string vs array), documented under **4. Type and value constraints → type**.
...
**Our implementation:** We store `examples` as `Option<Vec<serde_json::Value>>` on `JsonSchema` (serialized as `"examples"`). **Validator:** Parse and store only; no validation logic. **Codegen (schema → Rust):** Codegen emits examples in doc comments for generated structs and string enums (each example as a line under `# Examples`); no const emission. **Dedupe:** Full mode includes `examples` in the dedupe key (same shape with different examples yields two structs); Functional mode excludes it (different examples yield one struct). **Reverse codegen:** Serializing a `JsonSchema` that has `examples` emits them; hand-written structs do not set examples unless a future attribute is added. **allOf merge:** When merging object schemas, if the target has no examples we copy from the other subschema.
...
The `deprecated` keyword is a meta-data/annotation keyword (2019-09 and 2020-12 meta-data vocabulary). Type: boolean; default: false. It indicates that the instance or property should not be used; it does **not** affect validation. Draft-07 does not define `deprecated`; it was introduced in 2019-09.

**Our implementation:** We store `deprecated` as `Option<bool>` in the schema model. Validation: annotation only—instances that satisfy the rest of the schema remain valid; we do not emit deprecation warnings. Codegen: when a property, struct, or enum schema has `deprecated: Some(true)`, we emit `#[deprecated]` (or `#[deprecated = "description"]` when description is present) on the corresponding Rust field or type. Dedupe: included in **Full** mode only (like `description`/`comment`); omitted from Functional. Reverse codegen: field-level `#[to_json_schema(deprecated = true)]` emits `"deprecated": true` in the property schema.

**Spec version quirks:** Draft-07 does not define `deprecated`. It appears in the meta-data vocabulary for draft 2019-09 and 2020-12. We accept and store it regardless of `$schema`; default settings target 2020-12.
