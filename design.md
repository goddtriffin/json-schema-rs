...
- **Codegen (JSON Schema → Rust):** When a string property schema has `pattern`, the backend emits `#[to_json_schema(pattern = "...")]` (value escaped for Rust string literals). Same pattern as `min_length`/`max_length`.
- **Reverse codegen (Rust → JSON Schema):** `String::json_schema()` returns `"type": "string"` with no pattern. Field-level `#[to_json_schema(pattern = "...")]` on a `String` (or `Option<String>`) field feeds into `property_inserts`, producing a schema with the specified `pattern`.
