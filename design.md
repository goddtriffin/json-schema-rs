...
Codegen is built around a **swappable backend** trait: input is a slice of `JsonSchema`, output is one generated source buffer per schema. The trait `CodegenBackend` has a single method, `generate(&self, schemas: &[JsonSchema]) -> Result<Vec<Vec<u8>>, Error>`. The **CLI** matches on the language argument and calls the corresponding backend (e.g. `RustBackend::generate`); the library exposes the trait and concrete backends (e.g. `RustBackend`). The only implementation today is **Rust** (`RustBackend`), which emits serde-compatible Rust structs (one buffer per schema; no cross-schema dedupe yet). The public API is `generate_rust(schemas: &[JsonSchema]) -> Result<Vec<Vec<u8>>, Error>`; callers write each buffer to a file, `Vec<u8>`, or stdout as needed. **Adding another language:** implement `CodegenBackend` for a new type (e.g. `PythonBackend`), add a match arm in the CLI’s `run_generate` for the language name (case-insensitive), and update the "supported" text in the unsupported-language error message (e.g. "supported: rust, python").

**Codegen entry points:** (1) **CLI** — one or more INPUTs (file paths, directory paths recursively searched for `.json`, or `-` for stdin); required `-o` output directory. Schema file ingestion is a **standalone step**: the CLI attempts to read every schema file; each failure (open, read, or parse) is logged to stderr with the file path, and the command exits with failure only after all files have been attempted (no output is written if any ingestion fails). Generated output path components are **sanitized** so that file and directory names are valid Rust identifiers (e.g. hyphens replaced with underscores: `schema-1.json` → `schema_1.rs`, `sub-dir/` → `sub_dir/`). A `mod.rs` is emitted in each output directory, re-exporting the generated modules with `pub mod name;`, so the output tree is a valid Rust module hierarchy. Generated Rust code (struct names, field names, derives) adheres to Rust API Guidelines and project style. (2) **Library** — `generate_rust(schemas)` returns `Vec<Vec<u8>>`; one buffer per schema. (3) **Macro** — the `json-schema-rs-macro` crate provides `generate_rust_schema!(...)`, which runs at compile time and **inlines** the generated Rust at the call site (no file output). Macro input: one or more string literals (file paths relative to `CARGO_MANIFEST_DIR`, or inline JSON Schema strings). The macro calls `RustBackend::generate(&schemas)` once and wraps each buffer in a `pub mod … { … }`; module name from file stem for paths, or `schema_0`, `schema_1`, … for inline. Single-schema is the case of one input and one output buffer.
