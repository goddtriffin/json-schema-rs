...
- **repos/** — Cloned repos by language (e.g. `repos/rust/oxidecomputer-typify/`). Filled by `make research_clone` (Git clones plus the BelfordZ json_schema crate from crates.io). Use `REFRESH=1 make research_clone` to force re-download of the BelfordZ crate (the script removes it automatically). Gitignored.
